...
* a) Konstruktion des Paneldatensatzes
* DoFile vom FDZ des IAB (Stata Panel Syntax); some variables added.
...
cap log close
set more off
log using ${log}\01_extract_iab_ep.log, replace
use ${orig}\iabbp_1993.dta
label lang de
/*Variable für das entsprechende Jahr bilden
Umbenennung und Umkodieren der Variablen jeder Welle zu einer Variable*/
...
* added
rename a44b fhire
rename a09b93 d_grow
gen export=a07/100 if a07>=0
gen ict = (a12b>0 & !missing(a12b))
gen antvorl_vor=a08/100 if a08>=0
...
insource ertlag eigentum kammer grjahr abbau verlag gruppe einheit sonst eigen zukauf neugest reorg umwelt qualität ///
betr_and fhire d_grow export ict antvorl_vor
compress
save ${data}\iabbp_1993_panel_test.dta, replace
clear
...
use ${orig}\iabbp_1994.dta
label lang de

*Bei Längsschnittanalysen diesen Filter entfernen!
*keep if quer1994=="Q"
...
* added
rename b46b fhire
rename b16b94 d_grow
gen export=b14/100 if b14>=0
gen ict = (b17b>0 & !missing(b17b))
gen antvorl_vor=b15/100 if b15>=0

gen outsourc=1 if b03a==1 | b03b==1 | b03c==1
replace outsourc=0 if b03a!=1 & b03b!=1 & b03c!=1
count if outsourc==.
...
ertlag eigentum kammer grjahr abbau verlag gruppe einheit sonst eigen zukauf neugest reorg umwelt qualität betr_and fhire ///
d_grow export ict antvorl_vor
compress
save ${data}\iabbp_1994_panel_test.dta, replace
clear
...
*Werteübertragung organisatorische Änderungen über die Jahre
gen help=c26ab
...
* added
rename c44b fhire
rename c16b95 d_grow
gen export=c14/100 if c14>=0
gen ict = (c17b>0 & !missing(c17b))
gen antvorl_vor=c15/100 if c15>=0

gen outsourc=1 if c03a==1 | c03b==1 | c03c==1
replace outsourc=0 if c03a!=1 & c03b!=1 & c03c!=1
count if outsourc==.
...
geschart invest tech arbzeit tarif uebtarif lohn azubi bea_einf bea_qual beanw inhaber teilzeit tz_frau befrist ///
bef_frau einstell entlass betrrat single form bran_n99 bran_n00 offen ges_frau weiterb uebstund outsourc west insource ///
ertlag eigentum kammer grjahr abbau verlag gruppe einheit sonst eigen zukauf neugest reorg umwelt qualität help* betr_and ///
fhire d_grow export ict antvorl_vor
compress
save ${data}\iabbp_1995_panel_test.dta, replace
clear
...
rename d42b fhire
rename d1496 d_grow
gen export=d18/100 if d18>=0
gen ict = (d21b>0 & !missing(d21b))
gen antvorl_vor=d17/100 if d17>=0

gen outsourc=1 if d02a==1 | d02b==1 | d02c==1
replace outsourc=0 if d02a!=1 & d02b!=1 & d02c!=1
count if outsourc==.
...
betr_and fhire d_grow export ict antvorl_vor
compress
save ${data}\iabbp_1996_panel_test.dta, replace
clear
...
rename e52b fhire
rename e1497 d_grow
gen export=e18a/100 if e18a>=0
gen ict =(e26b>0 & !missing(e26b))
gen antvorl_vor=e17/100 if e17>=0

gen outsourc=1 if e02a==1 | e02b==1 | e02c==1
replace outsourc=0 if e02a!=1 & e02b!=1 & e02c!=1
count if outsourc==.
...
eigentum kammer grjahr abbau verlag gruppe einheit sonst eigen zukauf neugest reorg umwelt qualität betr_and fhire d_grow export ict ///
antvorl_vor
compress
save ${data}\iabbp_1997_panel_test.dta, replace
clear
...
rename f48b fhire
rename f1298 d_grow
gen export=(f15c+f15d)/100 if f15c>=0 & f15d>=0
replace export = f15c/100 if f15c>=0 & f15d<0
replace export = f15d/100 if f15c<0 & f15d>=0
gen ict =(f18b>0 & !missing(f18b))
gen antvorl_vor=f14/100 if f14>=0
ren f07f modgewbe

gen outsourc=1 if f02a==1 | f02b==1 | f02c==1
replace outsourc=0 if f02a!=1 & f02b!=1 & f02c!=1
count if outsourc==.
...
geschart invest tech arbzeit tarif uebtarif lohn azubi bea_einf bea_qual beanw inhaber teilzeit tz_frau befrist bef_frau ///
einstell entlass betrrat single form bran_n99 bran_n00 offen ges_frau weiterb uebstund outsourc west insource ertlag ///
eigentum kammer grjahr abbau verlag gruppe einheit sonst eigen zukauf neugest reorg umwelt qualität help* betr_and ///
fhire d_grow export ict antvorl_vor modgewbe
compress
save ${data}\iabbp_1998_panel_test.dta, replace
...
rename g29b fhire
rename g12 d_grow
gen export=(g16c+g16d)/100 if g16c>=0 & g16d>=0
replace export = g16c/100 if g16c>=0 & g16d<0
replace export = g16d/100 if g16c<0 & g16d>=0

gen ict =(g22b>0 & !missing(g22b))
gen antvorl_vor=g14/100 if g14>=0

gen     outsourc=1 if g02a==1 | g02b==1 | g02c==1
replace outsourc=0 if g02a!=1 & g02b!=1 & g02c!=1
count if outsourc==.
...
kammer grjahr abbau verlag gruppe einheit sonst eigen zukauf neugest reorg umwelt qualität betr_and fhire d_grow export ict antvorl_vor
compress
save ${data}\iabbp_1999_panel_test.dta, replace
clear

**************************************
********* Welle 2000 *****************
**************************************
use ${orig}\iabbp_2000.dta,clear
label lang de
...
rename h51 fhire
rename h13 d_grow
gen export=(h15c+h15d)/100 if h15c>=0 & h15d>=0
replace export = h15c/100 if h15c>=0 & h15d<0
replace export = h15d/100 if h15c<0 & h15d>=0

gen ict =(h18b>0 & !missing(h18b))
gen antvorl_vor=h17/100 if h17>=0
...
gen outsourc=1 if h02a==1 | h02b==1 | h02c==1
replace outsourc=0 if h02a~=1 & h02b!=1 & h02c!=1
count if outsourc==.
...
geschart invest tech arbzeit tarif uebtarif lohn azubi bea_einf bea_qual beanw inhaber teilzeit tz_frau befrist bef_frau ///
einstell entlass betrrat single form bran_n99 bran_n00 offen ges_frau weiterb uebstund outsourc west insource ertlag eigentum ///
kammer grjahr abbau verlag gruppe einheit sonst eigen zukauf neugest reorg umwelt qualität help* betr_and fhire d_grow export ict antvorl_vor ///
sonderz kapbeteil altvers gewbeteil sonstson

compress
save ${data}\iabbp_2000_panel_test.dta, replace
clear
...
rename i56 fhire
rename i08 d_grow
gen export=(i11c+i11d)/100 if i11c>=0 & i11d>=0
replace export = i11c/100 if i11c>=0 & i11d<0
replace export = i11d/100 if i11c<0 & i11d>=0


gen ict =(i24b>0 & !missing(i24b))
gen antvorl_vor=i12/100 if i12>=0

ren i70a uebtarif_unan
ren i70b uebtarif_fach
ren i70c uebtarif_ange
ren i70d uebtarif_qual


gen outsourc=1 if i02a==1 | i02b==1 | i02c==1
replace outsourc=0 if i02a~=1 & i02b!=1 & i02c!=1
count if outsourc==.
...
eigentum kammer grjahr abbau verlag gruppe einheit sonst eigen zukauf neugest reorg umwelt qualität betr_and fhire d_grow ///
export ict antvorl_vor uebtarif_unan uebtarif_fach uebtarif_ange uebtarif_qual
compress
save ${data}\iabbp_2001_panel_test.dta, replace
clear
...
rename j53 fhire
rename j07 d_grow
gen export=(j10c+j10d)/100 if j10c>=0 & j10d>=0
replace export = j10c/100 if j10c>=0 & j10d<0
replace export = j10d/100 if j10c<0 & j10d>=0

gen ict =(j13b>0 & !missing(j13b))
gen antvorl_vor=j11/100 if j11>=0

gen outsourc=1 if j02a==1 | j02b==1 | j02c==1
replace outsourc=0 if j02a~=1 & j02b!=1 & j02c!=1
count if outsourc==.
...
bef_frau einstell entlass betrrat single form bran_n99 bran_n00 offen ges_frau weiterb uebstund outsourc west insource ertlag ///
eigentum kammer grjahr abbau verlag gruppe einheit sonst eigen zukauf neugest reorg umwelt qualität betr_and fhire d_grow ///
export ict antvorl_vor
compress
save ${data}\iabbp_2002_panel_test.dta, replace
clear
...
rename k40 fhire
rename k10 d_grow
gen export=(k13c+k13d)/100 if k13c>=0 & k13d>=0
replace export = k13c/100 if k13c>=0 & k13d<0
replace export = k13d/100 if k13c<0 & k13d>=0

gen ict =(k16b>0 & !missing(k16b))
gen antvorl_vor=k14/100 if k14>=0

ren k75b orietarif

gen outsourc=1 if k02a==1 | k02b==1 | k02c==1
replace outsourc=0 if k02a~=1 & k02b!=1 & k02c!=1
count if outsourc==.
...
bef_frau einstell entlass betrrat single form bran_n99 bran_n00 offen ges_frau weiterb uebstund outsourc west insource ertlag ///
eigentum kammer grjahr abbau verlag gruppe einheit sonst eigen zukauf neugest reorg umwelt qualität betr_and fhire d_grow ///
export ict antvorl_vor orietarif
compress
save ${data}\iabbp_2003_panel_test.dta, replace
clear
...
rename l41 fhire
rename l10 d_grow
gen export=(l13c+l13d+l13e)/100 if l13c>=0 & l13d>=0 & l13e>=0
replace export = l13c/100 if l13c>=0 & l13d<0 & l13e<0 
replace export = l13d/100 if l13c<0 & l13d>=0 & l13e<0
replace export = l13e/100 if l13c<0 & l13d<0 & l13e>=0
replace export = (l13c+l13d)/100 if l13c>=0 & l13d>=0 & l13e<0
replace export = (l13c+l13e)/100 if l13c>=0 & l13d<0 & l13e>=0
replace export = (l13d+l13e)/100 if l13c<0 & l13d>=0 & l13e>=0

gen ict =(l15b>0 & !missing(l15b))
gen antvorl_vor=l14/100 if l14>=0

gen outsourc=1 if l02a==1 | l02b==1 | l02c==1
replace outsourc=0 if l02a~=1 & l02b~=1 & l02c~=1
count if outsourc==.
...
form eigentum grjahr bran_n00 outsourc tech abbau ertlag uebstund kammer west betr_and help* fhire d_grow export ict antvorl_vor
compress
save ${data}\iabbp_2004_panel_test.dta, replace
clear

**************************************
********* Welle 2005 *****************
**************************************
use ${orig}\iabbp_2005.dta
label lang de
...
gen export=(m12c+m12d+m12e)/100 if m12c>=0 & m12d>=0 & m12e>=0
replace export = m12c/100 if m12c>=0 & m12d<0 & m12e<0 
replace export = m12d/100 if m12c<0 & m12d>=0 & m12e<0
replace export = m12e/100 if m12c<0 & m12d<0 & m12e>=0
replace export = (m12c+m12d)/100 if m12c>=0 & m12d>=0 & m12e<0
replace export = (m12c+m12e)/100 if m12c>=0 & m12d<0 & m12e>=0
replace export = (m12d+m12e)/100 if m12c<0 & m12d>=0 & m12e>=0

gen ict =(m18b>0 & !missing(m18b))
gen antvorl_vor=m13/100 if m13>=0

ren m56 oeffnklau
ren m58a ausstarif


gen outsourc=1 if m02aa==1 | m02ab==1 | m02ac==1
replace outsourc=0 if m02aa~=1 & m02ab~=1 & m02ac~=1
count if outsourc==.
...
arbzeit uebstundv tarif uebtarif lohn betrrat single form eigentum grjahr bran_n00 outsourc tech abbau ertlag ///
uebstund kammer west betr_and fhire d_grow export ict antvorl_vor ausstarif oeffnklau
compress
save ${data}\iabbp_2005_panel_test.dta, replace
clear
...
gen export=(n09c+n09d+n09e)/100 if n09c>=0 & n09d>=0 & n09e>=0
replace export = n09c/100 if n09c>=0 & n09d<0 & n09e<0 
replace export = n09d/100 if n09c<0 & n09d>=0 & n09e<0
replace export = n09e/100 if n09c<0 & n09d<0 & n09e>=0
replace export = (n09c+n09d)/100 if n09c>=0 & n09d>=0 & n09e<0
replace export = (n09c+n09e)/100 if n09c>=0 & n09d<0 & n09e>=0
replace export = (n09d+n09e)/100 if n09c<0 & n09d>=0 & n09e>=0

gen ict =(n14b>0 & !missing(n14b))
gen antvorl_vor=n10/100 if n10>=0

gen outsourc=1 if n02aa==1 | n02ab==1 | n02ac==1
replace outsourc=0 if n02aa~=1 & n02ab~=1 & n02ac~=1
count if outsourc==.
...
uebstundv tarif uebtarif lohn betrrat single form eigentum grjahr bran_n00 outsourc tech abbau ertlag uebstund ///
kammer west betr_and fhire d_grow export ict antvorl_vor
compress
save ${data}\iabbp_2006_panel_test.dta, replace
clear
...
rename o41 fhire
rename o15 d_grow
gen export=(o10c+o10d+o10e)/100 if o10c>=0 & o10d>=0 & o10e>=0
replace export = o10c/100 if o10c>=0 & o10d<0 & o10e<0 
replace export = o10d/100 if o10c<0 & o10d>=0 & o10e<0
replace export = o10e/100 if o10c<0 & o10d<0 & o10e>=0
replace export = (o10c+o10d)/100 if o10c>=0 & o10d>=0 & o10e<0
replace export = (o10c+o10e)/100 if o10c>=0 & o10d<0 & o10e>=0
replace export = (o10d+o10e)/100 if o10c<0 & o10d>=0 & o10e>=0

gen ict =(o17b>0 & !missing(o17b))
gen antvorl_vor=o12/100 if o12>=0

ren o84b oeffnklau

gen outsourc=1 if o02a==1 | o02b==1 | o02c==1 |o02d==1 | o02e==1
replace outsourc=0 if o02a~=1 & o02b~=1 & o02c~=1 & o02d~=1 & o02e~=1
count if outsourc==.
...
tech abbau ertlag uebstund kammer west betr_and einstell help* fhire d_grow export ict antvorl_vor oeffnklau
compress
save ${data}\iabbp_2007_panel_test.dta, replace
clear

**************************************
********* Welle 2008 *****************
**************************************
use ${orig}\iabbp_2008.dta
label lang de
...
gen west=1 if wo2008==1
replace west=0 if wo2008==2

replace p01son07=p01son07+p01ger07 if p01ger07~=. | p01ger07~=-9
replace p01son08=p01son08+p01ger08 if p01ger08~=. | p01ger08~=-9

gen outsourc =1 if p02a==1 | p02b==1 | p02c==1
replace outsourc=0 if p02a~=1 & p02b~=1 & p02c~=1
count if outsourc==.


replace p15=6 if p09==4

replace p19=0 if p18e==1

gen p30ber_g=p30ber+p30uni if p30ber!=-9 & p30uni!=-9			
replace p30ber_g=p30ber if p30uni==-9 & p30ber!=-9
replace p30ber_g=p30uni if p30ber==-9 & p30uni!=-9

replace p31tz=0 if p31a==2
replace p31tz_f=0 if p31a==2
replace p32bef=0 if p32a==2
replace p32bef_f=0 if p32a==2
replace p41ges=0 if p39==2
replace p52ages=0 if p51==2
replace p54ges=0 if p53==2
replace p61=3 if p59==3
replace p61=-9 if p59~=1 & p59~=2 & p59~=3

recode p94 (2=1) (1=2)
recode bula2008 (7=18)

rename bula2008 bula
rename hr2008q  hrf_quer
rename p01svb07 svbv
rename p01svb08 svb
rename p01son07 son_vor
rename p01son08 sonstige
rename p01ges07 ges_vor
rename p01ges08 gesamt
rename p03 	insource
rename p09 	geschart
rename p10 	geschvol
rename p15  	ertrlagv
rename p19 	invest
rename p23 	tech
rename p30einf 	bea_einf
rename p30ber_g bea_qual
rename p30inh 	inhaber
rename p30aus 	azubi
rename p30anw 	beanw
rename p30ges_f ges_frau
rename p31tz 	teilzeit
rename p31tz_f 	tz_frau
rename p32bef 	befrist
rename p32bef_f bef_frau
rename p41ges 	einstell
rename p54ges 	entlass
rename p52ages 	offen
rename p59 	tarif
rename p61 	uebtarif
rename p63 	lohn
rename p74 	weiterb
rename p78 	arbzeit
rename p80a  	uebstundv
rename p86a 	betrrat
rename p86b 	betr_and
rename p89 	form
rename p91 	single
rename p92b 	kammer
rename p94 	eigentum
rename p95b 	grjahr
rename p97b 	bran_n00

* added
rename p40 fhire
rename p17 d_grow

gen export=p11c/100 if p11c>=0
gen ict =(p18b>0 & !missing(p18b))
gen antvorl_vor=p13/100 if p13>=0

replace grjahr=999 if p95a==1
...
keep idnum jahr hrf_quer hr03_08p hr00_08p hr07_08p bula ges_vor gesamt svbv svb son_vor sonstige /// 
insource geschart geschvol ertrlagv invest eigen zukauf neugest reorg verlag gruppe einheit umwelt qualität /// 
sonst bea_einf bea_qual inhaber azubi beanw ges_frau teilzeit tz_frau befrist bef_frau offen weiterb ///
entlass arbzeit uebstundv tarif uebtarif lohn betrrat single form eigentum grjahr bran_n00 outsourc ///
tech abbau ertlag uebstund kammer west betr_and einstell fhire d_grow export ict antvorl_vor
compress
save ${data}\iabbp_2008_panel_test.dta, replace
clear
